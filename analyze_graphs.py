...
class PartitionStats:
    ...
    def get_normalized_edge_range(self):
        edge_count = self.partition_edges_counter.values()
...
def plot_partition_stats(input_file, partition_stats):
    ...
    for partition_style in ["modulo", "range"]:
        x = [math.log(stats.partition_count, 2) for stats in partition_stats if stats.partition_style == partition_style]
        y = [stats.get_normalized_edge_range() for stats in partition_stats if stats.partition_style == partition_style]
        # print(x, y)
    ...
    for partition_style in ["modulo", "range"]:
        x = [math.log(stats.partition_count, 2) for stats in partition_stats if stats.partition_style == partition_style]
        y = [stats.get_inter_partition_edge_ratio() for stats in partition_stats if stats.partition_style == partition_style]
        # print(x, y)
        ax.plot(x, y, label="partition:" + partition_style)
...
def main():
    ...
    input_files = ["wordassociation-2011", "enwiki-2015", "twitter", "uk-2007-05"] # ["darwini-50m"]
    partition_counts_list = [2, 4, 8, 16, 32, 64, 128, 256, 512, 1024]
